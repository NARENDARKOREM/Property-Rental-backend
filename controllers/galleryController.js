...
const upsertGallery = async (req, res) => {
    ...
    try {
        if (id) {
            ...
            Object.assign(gallery, { pid, cat_id, img: imgPath || gallery.img, status, add_user_id });
            await gallery.save();
            res.status(200).json({ message: 'Gallery updated successfully', gallery });
        } else {
            // Create new gallery
            const gallery = await TblGallery.create({ pid, cat_id, img: imgPath, status, add_user_id });
...
const deleteGallery = async (req, res) => {
    ...
    try {
        const gallery = await TblGallery.findOne({ where: { id }, paranoid: false });
        if (!gallery) {
            return res.status(404).json({ error: 'Gallery not found' });
        }
