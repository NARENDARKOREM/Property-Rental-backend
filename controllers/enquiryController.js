...
const getEnquiries = async (req, res) => {
    ...
    try {
        ...
        if (prop_id) {
            ...
            return res.status(400).json({ error: 'Property ID or User ID is required' });
        }

        const result = await Promise.all(enquiries.map(async (enquiry) => {
        ...
        res.status(200).json({ message: 'Enquiry list retrieved successfully', enquiries: result });
    } catch (error) {
        res.status(500).json({ error: 'Internal server error', details: error.message });
    }
};
